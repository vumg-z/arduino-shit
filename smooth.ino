...
const int stepDelay = 25;     // Increased delay for slower movement

const int targetDistance = 7;  // Target distance in cm
...
void loop() {
  ...
  if (abs(error) > 0.1) { // Adjust only if the error is significant
  ...
  // Turn on the LED if the distance is close to 7 cm
  if (abs(error) < 1.0) {
    digitalWrite(ledPin, HIGH);  // LED on when distance is near target
  } else {
    digitalWrite(ledPin, LOW);   // LED off when distance deviates
  }
